...
class MOTPEMultiObjectiveSampler(BaseMultiObjectiveSampler):
    ...
    def _split_observation_pairs(
        ...
        if _SPLITCACHE_KEY in trial.system_attrs:
            ...
            while len(indices_below) + sum(nondomination_ranks == i) <= n_below:
                indices_below = np.append(
                    indices_below, indices[nondomination_ranks == i]
                )
                i += 1
            ...
            if subset_size > 0:
                ...
                reference_point = np.maximum(
                    np.maximum(
                        1.1 * worst_point,
                        0.9 * worst_point,  # case: value > 0, case: value < 0
                    ),
            ...
            study._storage.set_trial_system_attr(
                trial._trial_id, _SPLITCACHE_KEY, attrs
            )
    ...
    def _sample_loguniform(
        ...
        low = distribution.low
        high = distribution.high
        return self._sample_numerical(
            study, trial, low, high, below, above, is_log=True
        )
    ...
    def _sample_discrete_uniform(
        ...
        best_sample = (
            self._sample_numerical(study, trial, low, high, below, above, q=q)
            + distribution.low
        )
    ...
    def _sample_from_gmm(
        ...
        # MOTPE (original): use truncated Gaussian (truncnorm) + resampling
        ...
        # MOTPE (modified): use Gaussian (normal) + clipping to the boundary
        """
        samples = self._rng.normal(
            size=size, loc=mus[active], scale=sigmas[active]
        )
...
def _solve_hssp(
    ...
    contributions = [
        hypervolume([v]).compute(reference_point) for v in rank_i_loss_vals
    ]
    MARK_AS_SELECTED = -1
    hv_selected = 0
...
def _calculate_default_weights_below(
    lvals: np.ndarray, indices_below: np.ndarray
) -> np.ndarray:
